...
> macOS 下的安卓 ADB 综合工具箱 —— 双栏文件管理、投屏、应用管理、Fastboot 刷机与脱机修补，一应俱全。

**adbtool** 是一款运行在 macOS 上的安卓调试工具箱，通过 ADB/Fastboot 与手机交互，覆盖日常折腾的常用场景：

- 📁 **双栏文件管理**：左侧手机存储，右侧本地磁盘，拖拽即传，复制 / 剪切 / 粘贴 / 重命名一应俱全
- 🖥️ **投屏 / 截屏 / 录屏**：一键把手机画面投到电脑，或录制屏幕
- 📦 **应用管理**：查看用户 / 系统应用，卸载、启动、停止、清数据、装 APK
- ⚡ **Fastboot 刷机**：单分区刷入、`flash-all` 脚本、`payload.bin` 全量包、云提取，A/B 双槽自动处理
- 🛠️ **脱机修补**：离线给 boot / init_boot 打补丁，支持 Magisk、制作 GKI 镜像、KernelSU / SukiSU 内核级 Root
- 🔧 **系统工具**：无线 ADB 配对、系统开关、Logcat、重启（系统 / Recovery / Fastboot）

原生 UI（PyQt6），后台线程运行，界面不卡顿。
